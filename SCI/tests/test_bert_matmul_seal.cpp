...
void MatMul(BERTFCField &he_fc, int32_t input_dim, int32_t common_dim, int32_t output_dim) {
//   int num_cols = 1;
  ...
  for (int i = 0; i < input_dim; i++) {
    ...
    C[i].resize(output_dim);
    prg.random_mod_p<uint64_t>(A[i].data(), common_dim, 3);
  }

  cout << "prime: " << prime_mod << endl;
  INIT_TIMER;
  START_TIMER;
  he_fc.matrix_multiplication(input_dim, common_dim, output_dim, A, B, C, true,
                              true);
...
int main(int argc, char **argv) {
  ...
  BERTFCField he_fc(party, io);
  cout << "Before MatMul" << endl;
  MatMul(he_fc, input_dim, common_dim, output_dim);
