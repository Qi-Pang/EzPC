...
vector<Plaintext> LayerNormField::generate_cross_packing_masks(const FCMetadata &data) {
...
vector<Ciphertext> LayerNormField::rotation_by_one_depth3(const FCMetadata &data, const Ciphertext &ct, int k) {
...
// column-wise packing
vector<Ciphertext> LayerNormField::bert_efficient_preprocess_vec(vector<uint64_t> &input, const FCMetadata &data) {
...
vector<vector<Plaintext>> LayerNormField::bert_cross_packing_bias(const vector<vector<uint64_t>> &bias, const FCMetadata &data) {
...
vector<pair<vector<vector<Plaintext>>, vector<vector<Plaintext>>>> LayerNormField::bert_cross_packing_single_matrix(const vector<vector<vector<uint64_t>>> &weights, const FCMetadata &data) {
...
vector<vector<vector<Plaintext>>> LayerNormField::bert_softmax_v_packing_single_matrix(const vector<vector<vector<uint64_t>>> &weights, const FCMetadata &data) {
...
void LayerNormField::bert_cipher_plain_bsgs(const vector<Ciphertext> &cts, 
...
void LayerNormField::bert_cipher_cipher_cross_packing(const FCMetadata &data, const vector<Ciphertext> &Cipher_plain_result, const vector<Plaintext> &cross_masks, vector<Ciphertext> &results) {
...
vector<vector<vector<uint64_t>>> LayerNormField::softmax_mask(const FCMetadata &data) {
...
vector<vector<vector<Plaintext>>> LayerNormField::preprocess_softmax_s2(const uint64_t *matrix, const FCMetadata &data, vector<vector<vector<uint64_t>>> &mask) {
...
vector<Ciphertext> LayerNormField::preprocess_softmax_s1(const uint64_t *matrix, const FCMetadata &data) {
    int num_cts_per_res = data.image_size * data.image_size * 2 / data.slot_count; // 1 or 4
    int num_col_per_ct = data.slot_count / 2 / data.image_size; // 64 or 32
...
vector<vector<vector<Plaintext>>> LayerNormField::preprocess_softmax_v_r(const uint64_t *matrix, const FCMetadata &data) {
...
uint64_t* LayerNormField::client_S1_V_R(const uint64_t *softmax_s1, vector<Ciphertext> &V, const FCMetadata &data) {
...
void LayerNormField::bert_softmax_V(vector<Ciphertext> &softmax_s1, vector<vector<vector<Plaintext>>> &softmax_s2, vector<Ciphertext> &V, vector<vector<vector<Plaintext>>> &R, const FCMetadata &data, vector<Ciphertext> &result) {
...
uint64_t* LayerNormField::bert_postprocess_V(vector<Ciphertext> &cts, const FCMetadata &data, const bool &col_packing) {
...
uint64_t* LayerNormField::bert_cross_packing_postprocess(vector<Ciphertext> &cts, const FCMetadata &data, const bool &col_packing) {
...
vector<vector<uint64_t>> LayerNormField::gamma_x2_plain_server(vector<vector<uint64_t>> &x2_pt, vector<uint64_t> &gamma, vector<vector<uint64_t>> &sharing_r, const FCMetadata &data) {
    for (int i = 0; i < data.image_size; i++) {
...
vector<Ciphertext> LayerNormField::gamma_x1_server(vector<Ciphertext> &x1_ct, vector<Plaintext> &enc_gamma, const FCMetadata &data) {
